...
from orders
where amount < (select min(amount) from orders where city="delhi");


-- Find orders with amount equal to amount of 'Amit'
select count(*) as total_orders
from orders
where amount = (select amount from orders where customer_name="Amit");

-- Find orders with amount greater than average amount of Mumbai
select count(*) as total_orders
from orders
where amount > (select avg(amount) from orders where city="mumbai");

-- Find orders where amount matches any amount from Mumbai
select amount
from orders 
where amount in (select amount from orders where city="mumbai");

-- Find orders where amount is greater than all amounts from Chennai
select amount
from orders
where amount > all (select amount from orders where city ="chennai");

-- Find customers who placed orders in cities where amount > 8000 exists

select customer_name
from orders
where orders in (select customer_name from orders where amount > 8000);

-- Find orders whose amount is less than any amount from Delhi
select order_id
from orders
where amount< any (select amount from orders where city="delhi");  

-- Find orders from cities where any order is cancelled
select city
from orders
where order_status in(select order_status from orders where order_status = "cancelled"); 


-- Find second highest order amount
select amount
from orders
where amount = (select amount from  orders order by amount desc limit 1 offset 1 );
...
FROM orders
GROUP BY city
HAVING AVG(amount) >= ALL (SELECT AVG(amount) FROM orders GROUP BY city);

-- Find customers whose order amount is greater than their city average
SELECT customer_name, city, amount
FROM orders o
WHERE amount > (SELECT AVG(amount) FROM orders WHERE city = o.city);
